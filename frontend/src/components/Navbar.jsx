        ...
        <div className='flex items-center gap-4'>
            {
                ...
                : <button onClick={() => navigate('/login') } className='bg-blue-600 text-white px-8 py-3 rounded-full font-medium md-block cursor-pointer'> Create account</button>
            }

            {/* ------------- Mobile Menu ------------------*/}
            <img onClick={() => setShowMenu(true)} className='w-6 md:hidden' src={assets.menu_icon} alt="" />
            <div className={` ${showMenu ? 'fixed inset-0 w-full h-full z-[200]' : 'h-0 w-0'} md:hidden right:0 top:0 bottom:0 z-20 overflow-hidden bg-white transition-all`}>
                <div className='flex items-center justify-between px-5 py-6 '>
                    <img className='w-48' src={assets.logo} alt="" />
                    <img className='w-10' onClick={()=>setShowMenu(false)} src={assets.cross_icon} alt="" />
                </div>
                <ul className='flex flex-col items-center gap-2 mt-6 px-6 text-lg font-medium'>
